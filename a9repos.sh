...
#
# Installs common/base AlmaLinux 9 repositories/programs (x86_64 ONLY)
#
# WHAT
# ¯¯¯¯
...
# a digit away from being enabled. DNF can also be run with the extra
# switch --enablerepo=repo.name, essentially doing the same, as a one-
# shot type deal. More DNF info can be found using the 'man' command or
# at DNF's official webpage --> https://dnf.readthedocs.io
#
# WHERE
# ¯¯¯¯¯
# a9repos.sh code repository is hosted on GitHub at
#
#       https://github.com/webmarka/a9repos
#
...
# HOWTO
# ¯¯¯¯¯
# Preferably from a FRESHLY INSTALLED AlmaLinux 9 server, a9repos.sh can
# be called as superuser (root) using one of the following methods:
#
#  Method #1
#  ¯¯¯¯¯¯¯¯¯
#   - Executing straight from GitHub raw URL. Example:
#
...
#
#      root@host ~ # curl -4sLko- https://raw.githubusercontent.com/webmarka/a9repos/refactor-a9/a9repos.sh | bash -
#
#  Method #3
#  ¯¯¯¯¯¯¯¯¯
#   - Saving it to a location on the machine and executing it. Example:
#
#      root@host ~ # wget https://raw.githubusercontent.com/webmarka/a9repos/refactor-a9/a9repos.sh -O a9repos.sh
#      root@host ~ # bash a9repos.sh
#
# HOWTO/INSTALL NOTE
# ¯¯¯¯¯¯¯¯¯¯
# You *MUST* be superuser to run this - sudo works as well.
#
...
NODEJS_VERSION=${NODEJS_VERSION:-20}; # Updated to LTS version for AlmaLinux 9

# MariaDB - Must be a valid version number
#   Reference: https://mariadb.com/kb/en/library/mariadb-releases/
MARIADB_VERSION=${MARIADB_VERSION:-10.11}; # Updated to a stable version for AlmaLinux 9

# PHP - Must be a valid remi repo version
#   Reference: https://blog.remirepo.net/pages/Config-en
PHP_VERSION=${PHP_VERSION:-81};

# Repository URL used to fetch resources from
REPOURL='https://raw.githubusercontent.com/webmarka/a9repos/refactor-a9';
# URL to insert into generated files
BRAGURL='https://github.com/webmarka/a9repos';
...
function _showHEADER() {
	tput bold && cat <<- "__EOF__"
	    ...
	    ║          ╩╚═╚═╝╩  ╚═╝╚═╝.sh   ║
	    ║                               ║
	    ║ Basic AlmaLinux 9 Install Script ║
	    ╚═══════════════════════════════╝
...
function _importGPGKEYS() {
    ...
    rpm --import https://dl.fedoraproject.org/pub/epel/RPM-GPG-KEY-EPEL-9; sleep 0.01; # Updated for AlmaLinux 9
...
function _installREPOS() {
    ...
    dnf install -y epel-release; sleep 0.1; # Simplified for AlmaLinux 9
    dnf install -y https://www.elrepo.org/elrepo-release-9.el9.elrepo.noarch.rpm &>/dev/null; sleep 0.1;
    dnf install -y https://rpms.remirepo.net/enterprise/remi-release-9.rpm &>/dev/null; sleep 0.1;
    ...
    cat <<- __EOF__ | tee /etc/yum.repos.d/mariadb.repo &>/dev/null;
        ...
        [mariadb]
        name=MariaDB ${MARIADB_VERSION} repository for AlmaLinux \$releasever
        baseurl=https://yum.mariadb.org/${MARIADB_VERSION}/el\$releasever-amd64
        ...
        [mariadb-source]
        name=MariaDB ${MARIADB_VERSION} Source repository for AlmaLinux \$releasever
        baseurl=https://yum.mariadb.org/${MARIADB_VERSION}/el\$releasever-amd64/srpms
        gpgkey=https://yum.mariadb.org/RPM-GPG-KEY-MariaDB
        gpgcheck=1
        enabled=0
        skip_if_unavailable=1
    ...
    cat <<- __EOF__ | tee /etc/yum.repos.d/nginx.repo &>/dev/null;
        ...
        # Check it out at ${BRAGURL}
        [nginx-stable]
        name=NGINX stable repository for AlmaLinux \$releasever
        ...
        [nginx-mainline]
        name=NGINX mainline repository for AlmaLinux \$releasever
        ...
        [nginx-source]
        name=NGINX source repository for AlmaLinux \$releasever
    ...
    2>/dev/null echo 'skip_if_unavailable=1' >> /etc/yum.repos.d/nodesource-el9.repo;
    ...
    for R in {elrepo,elrepo-extras,elrepo-kernel,epel,mariadb,nginx-stable,nodesource,remi,remi-safe,remi-php${PHP_VERSION},yarn}; do
        dnf config-manager --enable ${R} &>/dev/null;
        echo -e "  - \033[32mRepository ${R} enabled\033[0;1m.\033[0m"; sleep 0.3;
    done

    # Fix an issue with epel repo having nginx
    dnf config-manager --setopt=epel.exclude=nginx* --save &>/dev/null;

    # Remove spaces around equal signs in repo files
...
function _updateSYSTEM() {
    # Clear/refresh DNF cache
    echo -ne "\033[1mCleaning DNF cache...\033[0m"; sleep 0.1;
    dnf clean all &>/dev/null; echo -e "\033[32m DONE\033[0;1m.\033[0m";
    echo -ne "\033[1mGenerating a new cache (please wait)...\033[0m"; sleep 0.1;
    dnf makecache &>/dev/null; echo -e "\033[32m DONE\033[0;1m.\033[0m";
    # System update
...
# Function to fix DNF to only use IPv4
function _fixYUMV6() {
    echo -e "\033[1mFixing 'potential' IPv6 problem with some repos...\033[0m"; sleep 0.1;
    cat <<- __EOF__ >> /etc/dnf/dnf.conf

        # This might not behave as intended, potentially lowering
        # your server's download speeds when updating packages...
        # If you notice many failed updates or slower speeds, comment this line
        ip_resolve=4
		__EOF__
    echo -e "  - \033[32mDNF 'potential' IPv6 problem fixed\033[0;1m.\033[0m"; sleep 0.3;
}

# Function that adds to DNF default configuration
function _addtoYUMCONF() {
    _fixYUMV6;
    echo -e "\033[1mAdding extra DNF configurations...\033[0m"; sleep 0.1;
    # Modify default options
    sed -i '/keepcache=/s/=.*/=1/' /etc/dnf/dnf.conf;
    sed -i '/debuglevel=/s/=.*/=4/' /etc/dnf/dnf.conf;
    sed -i '/installonly_limit=/s/=.*/=3/' /etc/dnf/dnf.conf;
...
function _installDEVEL() {
    ...
    case "${_DEVPACK}" in
        ...
        *)
            ...
            dnf -y --skip-broken groupinstall "Development Tools" &>/dev/null; # "Fedora Packager" not available in AlmaLinux 9
            dnf -y --skip-broken install LibRaw-devel bison-devel boost-devel bzip2-devel c-ares-devel giflib-devel glib2-devel glibc-devel gmp-devel json-glib-devel libcurl-devel libcap-devel libevent-devel libexif-devel libjpeg-turbo-devel libpcap-devel libpng-devel libsodium-devel libstdc++-devel libtiff-devel libyaml-devel ncurses-devel openssl-devel pcre-devel python3-devel systemd-devel xz-devel zlib-devel nasm yasm-devel bzip2-devel &>/dev/null;
...
function _installMARIADB() {
    ...
    case "${_MARIADB}" in
        ...
        *)
            ...
            [[ ${_MARIADB_INST} -eq 0 ]] && dnf -y install mariadb-server mariadb-client &>/dev/null || dnf -y reinstall mariadb-server mariadb-client &>/dev/null;
            echo -e "  - \033[32mMariaDB server installed\033[0m.";
            sed -i.a9repos.OLD 's/\[server\]/\[server\]\nbind-address = 127.0.0.1/g' /etc/my.cnf.d/mariadb-server.cnf; sleep 0.3;
...
function _installNGINX() {
    ...
    case "${_GINX}" in
        ...
        *)
            ...
            [[ ${_NGX_INST} -eq 0 ]] && dnf -y install nginx &>/dev/null || dnf -y reinstall nginx &>/dev/null;
            nginx -t &>/dev/null;
            [[ ${?} -eq 0 ]] && {
                _NGXPID="$(pidof nginx)";
...
function _installNODE() {
    ...
    case "${_NODENPM}" in
        ...
        *)
            ...
            [[ ${_NODE_INST} -eq 0 ]] && dnf -y --skip-broken install nodejs npm &>/dev/null || dnf -y --skip-broken reinstall nodejs npm &>/dev/null;
...
function _installCOMMON() {
	...
	dnf -y --skip-broken --disablerepo=elrepo install ${_COMMON_PACKAGES} &>/dev/null;
	dnf -y --skip-broken install ${_COMMON_PACKAGES} &>/dev/null;
...
function _createDOTFILES() {
	...
	for BASHRCFILE in ~/.bashrc /etc/skel/.bashrc; do
		#echo -e "\nexport HISTIGNORE='&:exit:x:l:history:editenv:[h ]*:[ \\\t]*:?:??:w -i:pwd:srcalias*:srcexports*:srcfunctions*:srccolors*:srcdot*:env:quit:popd*:ginx*:chownwww*:dnf clean*:dnf makecache*'" >> ${BASHRCFILE};
		...
		case "${_SSHP}" in
			...
			*)
				...
				if ! [[ -z ${_SSHPC} || ${_SSHPC} -eq 22 ]]; then
					...
					dnf -y install iptables ip6tables iptables-services &>/dev/null;
...
function _installYUMSTUFF() {
	...
	dnf --disablerepo=\* --enablerepo=base install yum-utils yum-plugin-fastestmirror yum-plugin-post-transaction-actions deltarpm -y &>/dev/null &&
...
sleep 2; clear;
echo -e "\n\033[1mDone with this AlmaLinux 9 auto install script.\033[0m";
